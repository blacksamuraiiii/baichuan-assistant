...
### v1.4 - 增强数据获取的鲁棒性 + 完整测试框架 🛠️
本次升级聚焦于提升大数据量下API数据获取的稳定性和鲁棒性，核心是引入了流式JSON解析库 `ijson`，并优化了流式处理的实现细节。同时，建立了完整的测试框架，确保代码质量和稳定性。

- **引入流式JSON解析**: 正式集成 `ijson` 库，以应对API一次性返回海量数据时可能导致的内存溢出问题。通过流式解析，程序可以逐条读取和处理数据，而无需将整个JSON响应加载到内存中。
- **健壮的流处理机制**: 恢复并优化了 `ChainedStream` 辅助类和预读取机制。这一设计专门用于处理非标准流式响应的服务器，通过预读一小部分数据来判断响应类型（成功或失败），然后再将预读的块与原始流无缝拼接，确保 `ijson` 解析器能够稳定工作，有效避免了 `premature EOF` 等错误。
- **完整测试框架**: 建立了包含58个测试用例的综合测试套件，覆盖单元测试、集成测试和端到端测试，测试通过率达到100%，为项目提供了可靠的质量保障。
- **代码质量提升**: 修复了日志系统、加密解密、字符编码等关键问题，显著提升了应用程序的稳定性和可靠性。
- **保留核心优化**: 继承了 `v1.3` 版本中智能分批、内存控制、即时清理等所有内存优化特性，确保在流式处理的各个环节都保持高效和稳定。
...
测试框架包含单元测试、主程序测试和集成测试，全面覆盖核心功能模块。
...
- **开发团队**: 扬州电信
- **开发者**: 黄维申 (18118256818)
- **技术架构**: Python + CustomTkinter + Pandas + 完整测试框架
